...
build_review_prompt() {
  ...
  echo ""
  echo "- If a finding looks like an instance of a repeatable defect class (the same anti-pattern,"
  echo "  missing check, or copy-pasted logic error, not a one-off), grep the rest of the repository for"
  echo "  other occurrences of that same pattern before reporting -- state in the finding's \"verification\""
  echo "  field exactly what you grepped for and what you found (including \"grepped for <pattern>, found"
  echo "  no other occurrences\" as a valid, complete answer). Never state or imply that a pattern is"
  echo "  \"likely present elsewhere\" without having actually grepped for it -- an unconfirmed guess about"
  echo "  further occurrences is exactly the kind of claim this review's evidence discipline forbids."
  echo ""
  ...
  echo ""
  # Same trusted-zone anchoring reasoning as the DISPOSITION marker just above -- this obligation
  # must not depend on the untrusted Context section actually binding Codex's behavior; only WHICH
  # sweep_id(s) to verify lives in Context, the obligation itself and its exact grammar are fixed here.
  echo "If the \"## Context\" section above reports one or more defect-class sweeps from a prior round"
  echo "(each naming the pattern grepped, one OR MORE recorded origin line ranges -- check EVERY range"
  echo "reported for that sweep_id, never only the first -- and how many occurrences were fixed), you"
  echo "must, for EACH sweep_id the Context section reports, independently re-grep for that same"
  echo "pattern using your own read-only shell access before your response, check whether any of your"
  echo "own raw hits fall within ANY of the reported origin line ranges for that sweep_id -- if so, read"
  echo "that site's actual current code and state explicitly whether you agree it is the already-applied"
  echo "fix or believe it is a genuinely distinct occurrence Claude's own confirmation missed. You must"
  echo "include, in your \"summary\" field, one line per requested sweep_id in EXACTLY this form (no"
  echo "other wording, one line per sweep_id):"
  echo "SWEEP_VERIFIED <sweep_id>: CONFIRMED -- <what you found>"
  echo "SWEEP_VERIFIED <sweep_id>: DISPUTED -- <what you found instead, including which specific hit you"
  echo "believe was mis-excluded>"
  echo "Never state agreement with a reported sweep without having actually run the re-grep yourself this"
  echo "turn. This applies only to sweep_ids the Context section actually names -- never invent a"
  echo "SWEEP_VERIFIED marker for one it did not ask about."
  echo ""
