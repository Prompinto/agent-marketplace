#!/usr/bin/env bash
# Scenario-specific assertions for scope-commit-sha, invoked by
# check-result.sh as `expect.sh <result.json>`. Schema structure (including
# the target.scope==commit => coverage-must-be-null conditional) is already
...
check() {
  ...
  if [ "$actual" != "$expected" ]; then
    echo "scope-commit-sha: FAIL -- $desc: expected [$expected], got [$actual]" >&2
    FAIL=1
  fi
}
...
check "target.scope" "$(jq -r '.target.scope' "$RESULT_FILE")" "commit"
...
if [ ! -f "$INVOCATION_LOG" ]; then
  echo "scope-commit-sha: FAIL -- expected fake-codex invocation log at $INVOCATION_LOG, not found" >&2
  FAIL=1
else
