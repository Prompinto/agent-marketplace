...
if [ ! -f "$JSONL_FILE" ]; then
  ...
  if [ -z "$ROUND1_CURRENT_LINE" ]; then
    ...
    if [ "$MATERIAL_RECEIPT_TYPE" != "string" ]; then
      echo "receipt-mismatch-phase2-reject: FAIL -- round-1 current-thread record: codex_review.material_receipt must be a JSON string, got type [$MATERIAL_RECEIPT_TYPE]" >&2
      FAIL=1
    else
