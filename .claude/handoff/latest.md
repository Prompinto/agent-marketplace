...
> **Stale as of 2026-09-10 — this is a snapshot from before a later PR (`04996d2`) removed Phase
> 5 Item A (`PROMPT_SIZE_LIMIT_BYTES`/`artifact_too_large`/`🛑 INPUT TOO LARGE`) entirely.** The
> Phase 5 table row below, and any mention of that preflight elsewhere in this file, describe the
> ORIGINAL Phase 5 shape, not the current one — see
> `docs/2026-09-05-codex-stream-review-improvement-roadmap-design.md`'s own "Superseded update
> (2026-09-10)" note under Phase 5 for what changed. This handoff file is regenerated by
> `clear-prep` on its next run; this note exists only so a reader who opens it before that happens
